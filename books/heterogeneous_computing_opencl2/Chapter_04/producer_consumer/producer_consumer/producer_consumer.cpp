...
int main(int argc, char **argv)
{
	...
	* data from disk */
	...
	gpuQueue = clCreateCommandQueueWithProperties(context, gpuDevice, 0, &status);
	check(status);
	cpuQueue = clCreateCommandQueueWithProperties(context, cpuDevice, 0, &status);
	check(status);
	...
	* pointer to the image data on the host. */
	...
	for (i = 0; i < HIST_BINS; i++) {
		if (hOutputHistogram[i] != refHistogram[i]) {
			cout << i << hOutputHistogram[i] << " != " << refHistogram[i] << endl;
			passed = 0;
		}
	}
	if (passed) {
