...
__global__
void renderKernel(uchar4 *d_out, float *d_vol, int w, int h, int3 volSize, int method, float zs, float theta, float threshold, float dist) {
    ...
    if (!hitBox) {
        ...
        if (t0 < 0.0f) {
            t0 = 0.f; // clam to 0 to avoid looking backward
        }
...
void kernelLauncher(uchar4 *d_out, float *d_vol, int w, int h, int3 volSize, int method, int zs, float theta, float threshold, float dist) {
