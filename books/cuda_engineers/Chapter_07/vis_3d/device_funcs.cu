...
__device__
float func(int c, int r, int s, int id, int3 volSize, float4 params){
...
bool intersectBox(Ray r, float3 boxmin, float3 boxmax, float *tnear, float *tfar){
    ...
    // find the largest tmin and smalles tmax
...
__device__
uchar4 sliceShader(float *d_vol, int3 volSize, Ray boxRay, float gain, float dist, float3 norm){
    float t;
...
__device__
uchar4 volumeRenderShader(float *d_vol, int3 volSize, Ray boxRay, float threshold, int numSteps){
    uchar4 shade = make_uchar4(96, 0, 192, 0); // background value
...
__device__
uchar4 rayCastShader(float *d_vol, int3 volSize, Ray boxRay, float dist){
    uchar4 shade = make_uchar4(96, 0, 192, 0); // background value
    float3 pos = boxRay.o;
