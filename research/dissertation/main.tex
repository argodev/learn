...
%%% files are read in automatically. All thes files must be available to PCTeX32.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    \title{Dimensionally Aligned Signal Projections\\for Signal Characterization} %type title with line breaks \\ in inverted pyramid style
    \author{Jason Michael Vann} %put your name here
    \degree{Doctor of Philosophy} %change to, for example, Master of Art if needed
    \department{Electrical Engineering} %this is your discipline
    \graduatedate{December 2017} %graduation date 
    \graduateyear{2017}    %graduation year                         
...
\def\gradcommitteechair{Adam Anderson}      %NOTE: Grad committee chair becomes
                                            %      co-chairperson when there is
                                            %      at least one co-chairperson
%\def\gradcommitteecochair{Rafal Ablamowicz}
\def\memberB{Jeffrey Austen}
\def\memberC{Wayne Johnson}
\def\memberD{Ghadir Radman}
\def\memberE{Doug Talbert}
...
\include{ModelDevelopment}   
\include{DataCollection}    
\include{DASPAlgorithms}    
\include{DASPFeatures}   
\include{DASPConfig}
\include{DASPClassification}     
\include{Conclusions}     
\include{references} 
\appendixnodots
\include{appendixHASPI}   
\include{appendixDASPImages} 
\include{vita}   
